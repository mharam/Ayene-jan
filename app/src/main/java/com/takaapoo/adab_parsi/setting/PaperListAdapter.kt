...
class PaperListAdapter(context: Activity) : ArrayAdapter<Int>(context, -1, arrayOf()) {

    private val paperColodIds = arrayOf(R.color.paper_1, R.color.paper_2, R.color.paper_3)
    private val paperNames = context.resources.getStringArray(R.array.paper_entries)
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        shape.setColor(ResourcesCompat.getColor(context.resources, paperColodIds[position], context.theme))

        binding.borderImage.setImageDrawable(shape)
        binding.borderImage.scaleType = ImageView.ScaleType.CENTER
        binding.borderName.text = paperNames[position]
        binding.borderRadioButton.isChecked = (position == sharedPreferences.getInt("paper_color", 0))
